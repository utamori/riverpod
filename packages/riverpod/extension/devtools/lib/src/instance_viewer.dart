...
int estimatedChildCount(EstimatedChildCountRef ref, InstancePath rootPath) {
  int estimatedChildCount(InstancePath path) {
    int expandableEstimatedChildCount(Iterable<PathToField> keys) {
      if (!ref.watch(isExpandedProvider(path))) return 1;

      return keys.fold(1, (acc, element) {
        return acc + estimatedChildCount(path.pathForChild(element));
      });
    }

    final instance = ref.watch(instanceProvider(path));

    return switch (instance) {
      AsyncValue(error: != null) => 1,
      AsyncValue(
        valueOrNull: NullInstance() ||
            BoolInstance() ||
            NumInstance() ||
            StringInstance() ||
            EnumInstance()
        ...
        1,
      AsyncValue(valueOrNull: final MapInstance instance) =>
        expandableEstimatedChildCount(
          instance.keys.map(
            (key) => PathToField.mapKey(ref: key.instanceRefId),
          ),
        ),
      AsyncValue(valueOrNull: final ListInstance instance) =>
        expandableEstimatedChildCount(
          List.generate(instance.length, PathToField.listIndex),
        ),
      AsyncValue(valueOrNull: final ObjectInstance instance) =>
        expandableEstimatedChildCount(
          instance.fields.map(
            (field) => PathToField.fromObjectField(field),
          ),
        ),
      AsyncValue(valueOrNull: null) => 1,
    };
  }
  ...
  Iterable<Widget> _buildMapItem(
    ...
    for (final key in instance.keys) {
      final value = _buildListViewItems(
        context,
        ref,
        path: path.pathForChild(PathToField.mapKey(ref: key.instanceRefId)),
      );
  ...
  Iterable<Widget> _buildListItem(
    ...
    for (var index = 0; index < instance.length; index++) {
      final children = _buildListViewItems(
        context,
        ref,
        path: path.pathForChild(PathToField.listIndex(index)),
      );
  ...
  Iterable<Widget> _buildObjectItem(
    ...
    for (final field in instance.fields) {
      ...
      final children = _buildListViewItems(
        context,
        ref,
        path: path.pathForChild(PathToField.fromObjectField(field)),
      );
